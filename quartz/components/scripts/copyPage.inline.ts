...
document.addEventListener("nav", () => {
  ...
  if (titleEl.querySelector(".copy-page-button")) return

  const button = document.createElement("button")
  button.className = "copy-page-button"
  button.type = "button"
  button.innerHTML = svgCopy
  button.ariaLabel = "Copy article HTML"
  button.title = "Copy article HTML"

  function onClick() {
    const article = document.querySelector("article")
    if (!article) return

    navigator.clipboard.writeText(article.innerHTML).then(
      () => {
        button.blur()
