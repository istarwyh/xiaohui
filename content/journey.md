...
- 2026 年 9 月 12 日，在上海浦东滨江喜来登酒店举行的 `AI Maker` 上海活动中分享《保险快查的 `Agent Native` 实践》。三个月前在高德，我讲的是 114 次自动迭代怎样逼出 `Harness`；这一次，我把共同执行内核、`Agent Protocol`、上下文资产、离线与在线交付，以及评测反馈如何进入下一版能力放进了同一套产品架构里。相关材料：[演示文稿与逐页讲稿](https://aispeeds.me/shares/agent-native-product-ai-maker-shanghai)、[[构建 Agent Native Product：保险快查离线与在线实践演讲稿（AI Maker 上海 45 分钟版）]]
