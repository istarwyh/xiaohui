...
modified: 2026-06-26T22:05:02+08:00
...
### 5.5. 免费的小狗：维护者为什么可以拒绝 PR
上面这些是贡献者视角：我如何进入社区，如何从小贡献开始积累功绩。但如果把视角翻到维护者一侧，提交 `PR` 并不总是“给项目送来了免费劳动力”。SQLite 作者 Richard Hipp 在一次访谈里用过一个很形象的说法：Linus 常谈 `free as in beer` 与 `free as in speech`，但开源世界还有第三种 `free`，叫 `free as in puppies`。一个 `PR` 看起来免费，真正被接受后，它就变成了项目的一部分：以后要测试、要文档、要兼容、要响应 `bug`，还要和未来很多年的架构一起活下去。贡献者可以轻盈离开，维护者却要长期照顾它。相关转录见 [Pull Requests are Free Puppies](https://lobste.rs/s/aqk7vl/pull_requests_are_free_puppies)。

所以开源社区不是谁提 `PR` 谁天然占据道德高地。提交贡献是一种善意，拒绝贡献也是维护项目边界的一部分。尤其在 `AI` 能低成本生成大量“看起来能跑”的代码之后，维护者面对的往往不是代码数量不足，而是判断、审查和长期背责的成本被放大。低质量 `PR`、`KPI` 式贡献、只改拼写或堆功能的 `PR`，本质上都可能把“社区共建”变成对维护者时间的转嫁。[[假如Linus在中国]]里讽刺的，正是这种贡献热情和维护成本不对称的荒诞。

真正好的开源贡献，应该把“小狗”的口粮也一起带来：清楚的动机、可复现的问题、足够小的变更、测试、文档、后续响应意愿，以及对项目方向的尊重。否则 `PR` 只是把“我想要的功能”包装成“我送你的礼物”。对维护者来说，能说“不”不是冷漠，而是项目能够长期活下去的必要能力。
