...
**从题目到思路** 通过手动模拟正确的出入栈操作得到思路: 遍历`pop序列`,也就是正在测试的出栈序列:

1. 如果要弹出的这个元素在栈顶-->弹出
2. 如果要弹出的这个不在栈顶, - 按压入顺序将下一个压入,一直压到这个数字再弹出 - 如果压不到这个数字/压入弹出不对,说明出栈序列不对<--翻译成**code语言**就是： <center>如果是空，就返回false;否则继续压入</center> **从步骤到代码**
