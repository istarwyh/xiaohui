...
宠物商城在最外层呈现为用户端，宠物商城调用宠物后端，宠物后端从营销、搜索、风控推荐三个渠道获取宠物商品信息这些信息会进行整合和标准化处理

## 特征构建模块（左侧框）

离线特征包含三类基础特征：user特征、item特征和user2item特征实时特征包含用户实时浏览商品数据和购买商品数据
...
对商品进行多维度过滤：疲劳度过滤互斥过滤（与互斥队列相关）商品状态过滤
