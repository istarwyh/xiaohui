...
搜索 `Agent` 最多执行三轮 `ReAct` 工具调用。条件明确时，所有已知条件通常一次写入 `concurrent_search` 的 `search_tasks`；工具结果过宽、过窄或还需补充约束时，`Agent` 可以继续调用。继续收窄时，下一轮用 `scope="last"` 在上一轮尚未展示截断的完整结果池上追加谓词，不重复旧条件。任务中包含核保查询时，整个搜索只执行一轮。
