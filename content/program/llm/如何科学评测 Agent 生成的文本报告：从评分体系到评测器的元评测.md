...
说白了：不但要评 `Agent`，也要评“评测 `Agent`”。这和 `Agent` 评测研究里的趋势是一致的：只看最终答案越来越不够，规划、工具、记忆、轨迹、安全、鲁棒性都开始进入评测视野。[^agent-eval-survey]

## 一、先说业务背景：评的是产物，不是 Agent 的智商
...
所以它不适合只用一个整体分数来评。一个总分只能告诉你“好像不太行”，但无法告诉你：

> 到底是哪一类能力不行？

## 二、第一层：如何评测文本报告？

比较靠谱的方式不是直接打总分，而是把报告评测拆成三层：
...
这个思想并不新。机器翻译评测里的 `MQM` / `WMT` 人工评测路线，本质上也是显式标注错误类型和严重度，再通过权重聚合成质量判断。[^mqm] 复杂业务报告只不过把翻译错误类型换成了业务错误类型：数字错、引用缺失、事实幻觉、合规风险、逻辑矛盾、关键信息遗漏。
...
因为评测器一旦结构化，它就从“点评器”变成了“测量仪器”。

点评器只能看。测量仪器可以校准、回归、监控、优化。

`G-Eval` 用 `CoT` 和表单化评分提升 `LLM` 评价和人类评价的一致性，`Prometheus 2` 则代表了训练专门评测模型、支持自定义 `rubric` 的路线。[^g-eval] [^prometheus] 它们共同说明一件事：`LLM Judge` 不是不能用，但必须被放进明确的评分表、证据要求和输出格式里。
...
`MetaCritique` 这类工作已经把 critique 拆成原子信息单元，再在原子层面计算事实性、完整性和 `F1`。[^metacritique] 这个方向和文本报告评测非常像：不要只看大段评论是否顺眼，要看每个原子判断有没有复现专家判断。
...
> `ESF` 不是看评测器会不会猜分，而是看它有没有学会专家在哪里扣分、在哪里放行。
...
一个评测器如果第一次给 82，第二次给 91，第三次又说合规不通过，那它就不是评测系统，而是随机点评器。
...
近年的 `LLM-as-a-Judge` 可靠性研究也在把 consistency 和 human alignment 分开看：一个评测器既要和人类专家接近，也要在等价评测条件下保持稳定。[^llm-judge-survey] [^judge-irt] 这也是为什么我把 `REC` 单独拿出来，而不是只看和专家分数的相关性。
...
> 用专家 `GT` + 构造样本训练评测器，用独立验证集上的 `ESF`、`SCE`、`REC` 验证它是否真正复现专家判断逻辑并保持稳定。
...
> 评测 `Agent` 生成的文本报告，不要从总分开始，而要从 `checkpoint` 开始；不要只评产物，也要评评测器；不要只看像不像专家，还要看是否稳定、是否能在构造样本上泛化。
...
这也是为什么我认为，在 `Agent` 系统里，评测不是配套工具，而是核心基础设施。[[从RAG到DeepResearch：复杂业务报告生成的上下文工程]] 解决的是报告如何生成，[[Harness Engineering - C 端 AIGC 实时生成系统]] 解决的是生成产物如何进入生产闭环，而这篇文章解决的是：当我们说“质量变好了”的时候，这把尺子本身到底靠不靠谱。
