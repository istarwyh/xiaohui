...
modified: 2026-06-05T00:00:00+08:00
...
如果说普通 `RAG` 像是把一摞资料搬到写作者桌上，那么 `Deep Research` 更像是派出一位研究员：他不会从第一页读到最后一页就开始写，而是会先画出问题地图，沿着线索走访关键地点，遇到矛盾时折返，发现盲区时补查，最后把这段调查路线沉淀成报告。
...
复杂业务报告更需要后者。因为报告里的关键判断，往往不是由某一段文本单独决定，而是由一连串材料之间的关系决定。
...
这个判断不是最终结论，而是第一版路线图。接下来的搜索，就是不断验证这张地图哪里准确、哪里过度简化、哪里画漏了悬崖。
...
这一步的核心是：系统要主动搜索可能让当前结论变弱的材料。它不只是顺着大路往前走，还要刻意绕到背街小巷看看：有没有一条不起眼的岔路，会把原来的结论带向完全不同的地方。
