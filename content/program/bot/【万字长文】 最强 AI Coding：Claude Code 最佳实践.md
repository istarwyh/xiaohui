...
> ⚠️ **过度探索陷阱** — 最常见的失败模式，本质是 [[上下文工程|上下文污染]]（context rot）。触发方式有两种：一是架构总览写太多，Agent 读到大段架构描述后一路打开几十个文档，上下文塞进几十万 token，反而写出更差的代码；二是"别做"清单太长（30–50 条禁止项堆在一起，没有对应的"该做什么"），Agent 开始逐条核查自己的方案是否违反每条规定——哪怕当前任务根本不涉及那些场景。详见 [[从Claude Code到 OneAgent：如何做好上下文工程]] 中的"上下文压缩"策略。
