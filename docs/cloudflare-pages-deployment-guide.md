...
1. **@napi-rs/simple-git 依赖问题**:
   - 问题: 缺少平台特定的二进制模块
   - 解决: 添加相应平台的依赖到 `optionalDependencies`

2. **yargs 版本冲突**:
   - 问题: 依赖和 override 版本不一致
   - 解决: 统一 override 版本到 `^18.0.0`

3. **Node.js 版本兼容性**:
   - 问题: yargs 需要 Node.js `^20.19.0 || ^22.12.0 || >=23`
   - 解决: 使用 `engine-strict=false` 绕过版本检查

4. **npm ci 与 package-lock.json 同步问题**:
   - 问题: 添加新依赖后 lock 文件未更新
   - 解决: 运行 `npm install` 更新 lock 文件并提交
